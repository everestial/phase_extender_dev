...
def groupby_and_read(file_path, bed_file, soi, snp_threshold, sample_list, num_of_hets, lods_cut_off, maxed_as, writelod):
    ...
    if bed_file:
        ...
        for bed_id, data_by_bed in good_data_by_bed:
            if 'void' in bed_id:
                ...
                phase_extended_by_bed =  process_consecutive_blocks(contigs_group, soi, chr_, snp_threshold,
    ...
    # ..just pass whole contig/chromosome at once
        ...
        phase_extended_by_chr = process_consecutive_blocks(contigs_group, soi, chr_, snp_threshold,
                               sample_list, num_of_hets, lods_cut_off, writelod, maxed_as)
...
def process_consecutive_blocks(contigs_group, soi, chr_, snp_threshold,
    ...
    for (k1, v1), (k2, v2) in zip(grouped_data.items(), itertools.islice(grouped_data.items(), 1, None)):
        ...
        for m in range(len(v2[soi + ':PI'])):    # m-ranges from block02
            ...
            for x, y in sample_list:
                ...
                # if PI values are not same then we create product between phased nucleotides
                    ...
                    prod_nuclb1b2 = list(itertools.product(nucl_B1, nucl_B2))
